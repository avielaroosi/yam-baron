...
### Task 8: Brand logo integration (executed right after Task 2; owner supplied the logo on 2026-09-23)

**Files:**
- Already on disk (uncommitted, generated by the controller from the owner's PNG): `assets/brand/logo-original.png` (owner's file, cream background), `assets/brand/logo-mark.png` (gold YB monogram, transparent), `assets/brand/logo-full.png` (monogram + black "YAM BARON / HAIR STUDIO", transparent, for cream backgrounds), `assets/brand/logo-full-light.png` (same with cream text, for dark backgrounds), `assets/favicon.png` (256×256 monogram, transparent).
- Delete: `assets/favicon.svg` (replaced by the PNG)
- Modify: `js/content.js` (add `logo` block), `tools/check-content.mjs` (validate it), `index.html` (hero, head, footer), `css/style.css` (hero logo, footer, wordmark font), `tools/check-page.mjs` (assertions)

**Interfaces:**
- Consumes: `#hero-name`, `#hero-sub`, `#footer-name`, `.wordmark`, `--font-mark` from Task 2.
- Produces: `S.logo = { hero, mark }`; `<img id="hero-logo">`, `<img id="footer-logo">`. `#hero-name` and `#hero-sub` stay in the DOM (SEO + harness) but become visually hidden.

- [ ] **Step 1: Failing assertions**

Insert into `tools/check-page.mjs` immediately before `// --- WhatsApp links`:
...
And in `tools/check-content.mjs`, after the `about` check add:

```js
need(S?.logo?.hero && S?.logo?.mark, "SITE.logo needs {hero, mark}");
```
and add `S?.logo?.hero, S?.logo?.mark,` to the `files` array (so existence is verified).

Run: `cd tools && node check-content.mjs; node check-page.mjs` → both FAIL (`SITE.logo needs {hero, mark}`; `hero: logo image src`).

- [ ] **Step 2: Content + markup**

`js/content.js` — add after `hero: {...},`:
...
`index.html`:
- In `<head>`, replace the favicon line with:
  ```html
  <link rel="icon" href="assets/favicon.png" type="image/png" sizes="256x256">
  <link rel="apple-touch-icon" href="assets/favicon.png">
  ```
- Fonts link: replace `Cormorant+Garamond:wght@400;500;600` with `Playfair+Display:wght@400;500;600` (the logo's lettering is a high-contrast Didone; Playfair matches it, Cormorant does not).
- In `.hero__inner`, replace the two lines `<p class="hero__eyebrow" id="hero-sub"></p>` and `<h1 class="wordmark" id="hero-name"></h1>` with:
  ```html
  <img class="hero__logo" id="hero-logo" alt="">
  <h1 class="sr-only" id="hero-name"></h1>
  <p class="sr-only" id="hero-sub"></p>
  ```
- Footer: replace `<p><span class="wordmark wordmark--sm" id="footer-name"></span> · <span id="footer-year"></span></p>` with:
  ```html
  <img class="footer__logo" id="footer-logo" alt="">
  <p><span class="wordmark wordmark--sm" id="footer-name"></span> · <span id="footer-year"></span></p>
  ```

`js/main.js` — in `renderHero()` add: `const logo = $("hero-logo"); logo.src = S.logo.hero; logo.alt = S.name + " " + S.sub;` (keep the `hero-name`/`hero-sub` textContent lines). In `renderFooter()` add: `const fl = $("footer-logo"); fl.src = S.logo.mark; fl.alt = S.name; fl.loading = "lazy";`.
...
- Append:
  ...
  .hero__logo { width: min(78vw, 400px); height: auto; filter: drop-shadow(0 6px 24px rgba(0, 0, 0, .45)); }
  .footer__logo { width: 44px; height: auto; margin: 0 auto 10px; opacity: .9; }
  ```
- Remove the now-unused `.hero__eyebrow` rule and `.hero .wordmark` rule (the h1 is sr-only now). Keep `.wordmark` / `.wordmark--sm` (footer).

Delete `assets/favicon.svg`.

- [ ] **Step 3: Run checks, look, commit**

Run: `cd tools && node check-content.mjs && node check-page.mjs` → both OK. Open `tools/shots/mobile-fold.png`: the gold monogram with cream lettering sits centered over the photo, tagline and WhatsApp button below it, all above the fold. Open `tools/shots/desktop.png`: footer shows the small monogram above "YAM BARON · 2026".

```bash
git add assets/brand assets/favicon.png js/content.js js/main.js index.html css/style.css tools/check-content.mjs tools/check-page.mjs
git rm -q assets/favicon.svg
git commit -m "feat: integrate brand logo (hero, footer, favicon); Playfair wordmark"
```
(append the Co-Authored-By / Claude-Session trailer lines from Global Constraints.)
